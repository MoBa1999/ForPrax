...
class Test_Model(nn.Module):
    def __init__(self, input_length, tar_length, classes = 5, conv_1_dim = 10, conv_2_dim = 20,attention_dim =40,
                  tar_len_multiple=2, num_reads = 1, n_heads = 8, at_layer =1, kernel_2 = 3, kernel_1 = 1, kernel_1_2 = 9):
        super(Test_Model, self).__init__()

        
        self.conv1 = nn.Conv2d(in_channels=1, out_channels=conv_1_dim, kernel_size = (num_reads,kernel_1), stride=1, padding='same')
        self.conv1_2 = nn.Conv2d(in_channels=conv_1_dim, out_channels=conv_1_dim, kernel_size = (num_reads,kernel_1_2), stride=1, padding='same')
        self.conv2 = nn.Conv2d(in_channels=conv_1_dim, out_channels=conv_2_dim, kernel_size = (num_reads,kernel_2), stride=1, padding='same')
        ...
        self.conv1d_3 = nn.Conv1d(in_channels=conv_2_dim*num_reads, out_channels=attention_dim, kernel_size=3, padding = 1)
    ...
    def forward(self, x):
        ...
        x = self.max_pool(x)
        x = x.reshape(batch_size,x.shape[1] * self.nr_reads,x.shape[3])
        x = self.conv1d_3(x)
